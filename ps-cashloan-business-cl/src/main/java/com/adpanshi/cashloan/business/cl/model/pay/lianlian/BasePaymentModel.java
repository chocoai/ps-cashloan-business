...
public class BasePaymentModel {
	...
	public BasePaymentModel(String service) {
		super();
		String businessno=LianlianServiceEnum.SERVICE.BANK_CARD_BIND.getName().equals(service)?LianLianConstant.BUSINESS_NO:LianLianConstant.BUSINESS_NO_R;
		this.setOid_partner(Global.getValue(businessno));
		this.setSign_type(LianLianConstant.SIGN_TYPE);
	}
	
	
	//TODO  绑定(用的是闪银的商户号)、放款(用的是齐融的)
	...
	public boolean verfiySign(BasePaymentModel model) {
		...
		logger.info("验签明文："+verifyJsonStr);
		String publicKey=LianlianServiceEnum.SERVICE.BANK_CARD_BIND.getName().equals(model.getService())?LianLianConstant.PUBLIC_KEY:LianLianConstant.PUBLIC_KEY_R;
		return SignUtil.checksign(Global.getValue(publicKey), verifyJsonStr,
				model.getSign());
	}
	 ...
	 * 
	 * @return com.adpanshi.cashloan.api.com.adpanshi.cashloan.api.cr.service
	 */
