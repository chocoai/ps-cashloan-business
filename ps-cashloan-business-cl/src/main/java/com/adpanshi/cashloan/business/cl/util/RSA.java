...
public class RSA{
	private static final String clipperInstance = "RSA/ECB/PKCS1Padding";
    public static void main(String ...args){
        RSA rsa = RSA.create();
        String pubKey = rsa.getPublicKey();
        String priKey = rsa.getPrivateKey();
        System.out.println(pubKey+"---你猜----"+priKey);
        //原文
    ...
    public String decodeByPublicKey(String res,String key){
        ...
        if(rs!=null){
            int len = 0;
            //组合byte[]
            byte[] result = new byte[rs.length*128];
    ...
    private String encodePub(byte[] res,byte[] keyBytes){
        ...
        try {
           ...
           // Cipher cp = Cipher.getInstance("RSA/None/NoPadding",new BouncyCastleProvider());//生成相应的Cipher
            Cipher cp = Cipher.getInstance(clipperInstance);//生成相应的Cipher
            System.out.println(kf.getAlgorithm()+"---kf.getAlgorithm()");
    ...
    private String encodePri(byte[] res,byte[] keyBytes){
        ...
        try {
            ...
            Cipher cp = Cipher.getInstance(clipperInstance);
